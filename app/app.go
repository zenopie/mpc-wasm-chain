...
import (
	...
	tssmodule "mpc-wasm-chain/x/tss/module"
	tsskeeper "mpc-wasm-chain/x/tss/keeper"
	tsstypes "mpc-wasm-chain/x/tss/types"
	tssabci "mpc-wasm-chain/x/tss/abci"
	tss "mpc-wasm-chain/x/tss"
)
...
type WasmApp struct {
	*baseapp.BaseApp
	...
	// TSS keeper (merged from MPC + Signing)
...
func NewWasmApp(
	...
	keys := storetypes.NewKVStoreKeys(
		...
		// tss module (merged from mpc + signing)
		tsstypes.StoreKey,
	)
	...
	*/
	...
	// Initialize TSS Keeper (merged from MPC + Signing)
	// Must be created before WasmKeeper to register custom message encoders and query plugins
	app.TssKeeper = tsskeeper.NewKeeper(
		runtime.NewKVStoreService(keys[tsstypes.StoreKey]),
		appCodec,
		app.AccountKeeper.AddressCodec(),
	...
	// Set node home for FROST key persistence and load any existing keys
	// Note: Errors are silently ignored during CLI commands when keys directory doesn't exist
	tsskeeper.SetNodeHome(homePath)
	_ = tsskeeper.LoadFROSTKeyShares()
	...
	// Register TSS custom message encoders and query plugins for WASM contracts
	tssWasmOpts := []wasmkeeper.Option{
		wasmkeeper.WithMessageEncoders(tss.CustomMessageEncoders(&app.TssKeeper)),
		wasmkeeper.WithQueryPlugins(tss.CustomQueryPlugins(&app.TssKeeper)),
	}
	wasmOpts = append(tssWasmOpts, wasmOpts...)
	...
	// Set the wasm keeper on the TSS keeper for contract callbacks
	app.TssKeeper.SetWasmKeeper(&app.WasmKeeper)
	...
	app.ModuleManager = module.NewManager(
		...
		// tss module (merged from mpc + signing)
		tssmodule.NewAppModule(appCodec, app.TssKeeper, app.AccountKeeper, app.BankKeeper),
	)
	...
	app.ModuleManager.SetOrderEndBlockers(
		...
		icatypes.ModuleName,
		wasmtypes.ModuleName,
		// tss module needs EndBlocker for DKG and signing state machine progression
		tsstypes.ModuleName,
	)
	...
	// Set up TSS vote extension and proposal handlers
	app.setTSSHandlers()
...
func (app *WasmApp) setTSSHandlers() {
