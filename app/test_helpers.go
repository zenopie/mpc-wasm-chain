...
func NewTestNetworkFixture() network.TestFixture {
	...
	appCtr := func(val network.ValidatorI) servertypes.Application {
		// Patch genesis file to enable vote extensions BEFORE CometBFT reads it
		patchGenesisForVoteExtensions(val.GetCtx().Config.RootDir)
...
var genesisPatched = false

// patchGenesisForVoteExtensions patches ALL genesis.json files to enable vote extensions
// This is called from AppConstructor BEFORE CometBFT reads the genesis
func patchGenesisForVoteExtensions(rootDir string) {
	// Only patch once - find and patch all genesis files in the testnet directory
	if genesisPatched {
		return
	}
	genesisPatched = true

	// rootDir is like .testnets/test-chain/node0/simd
	// We need to find all nodeN directories and patch their genesis files
	baseDir := filepath.Dir(filepath.Dir(rootDir)) // .testnets/test-chain

	// Find all node directories
	entries, err := os.ReadDir(baseDir)
	if err != nil {
		fmt.Printf("Warning: could not read base dir %s: %v\n", baseDir, err)
		return
	}

	for _, entry := range entries {
		if !entry.IsDir() || len(entry.Name()) < 4 || entry.Name()[:4] != "node" {
			continue
		}

		genesisFile := filepath.Join(baseDir, entry.Name(), "simd", "config", "genesis.json")
		if err := patchSingleGenesis(genesisFile); err != nil {
			fmt.Printf("Warning: failed to patch %s: %v\n", genesisFile, err)
		} else {
			fmt.Printf("SUCCESS: Patched %s for vote extensions\n", genesisFile)
		}
	}
}

func patchSingleGenesis(genesisFile string) error {
