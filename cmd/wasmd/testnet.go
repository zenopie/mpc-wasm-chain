...
Example:
	...
	cmd.Flags().Duration(flagCommitTimeout, 5*time.Second, "Block commit timeout (controls block time)")
	return cmd
}
...
func initGenFiles(
	...
	// Enable vote extensions for TSS - this is critical for DKG/signing to work
	// Vote extensions must be enabled at height 2 (height 1 needs to pass first)
	defaultParams := cmttypes.DefaultConsensusParams()
	defaultParams.ABCI.VoteExtensionsEnableHeight = 2
...
// startTestnet starts an in-process testnet with vote extensions enabled
func startTestnet(cmd *cobra.Command, args startArgs) error {
	// Use our custom implementation that properly enables vote extensions
	return startTestnetCustom(cmd, args)
}
